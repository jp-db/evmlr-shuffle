...
void discrete_gaussian_ctx_init(discrete_gaussian_ctx_t ctx, double sigma) {
	ctx->sigma = sigma;
	ctx->discrete_normalisation = 0.5 * M_SQRT1_2 * M_2_SQRTPI * (1.0 / sigma);
	ctx->sigma_inv = -1.0 / (2 * sigma * sigma);
	ctx->v_sigma_inv = _mm_setr_pd(ctx->sigma_inv, 0.0);
	ctx->v_discrete_normalisation = _mm_setr_pd(ctx->discrete_normalisation, 0.0);
	ctx->head = NORM_BATCH;
	memset(ctx->norm, 0, sizeof(ctx->norm));
}

void discrete_gaussian_ctx_clear(discrete_gaussian_ctx_t ctx) {
	memset(ctx, 0, sizeof(discrete_gaussian_ctx_struct));
}

/* Box-Muller sampler */
static inline void box_muller(discrete_gaussian_ctx_struct *ctx)
{
	...
	r2_cos.store(ctx->norm);
	r2_sin.store(ctx->norm + 4);
}

/* FACCT exp(x) */
static inline __m128d exp_constant(const __m128d x, const discrete_gaussian_ctx_struct *ctx)
{
	__m128d vx, vx1, vx2, vsum, vres;
	__m128i vt;

	vx = _mm_mul_sd(x, ctx->v_sigma_inv);
...
int64_t discrete_gaussian(const double center, discrete_gaussian_ctx_t ctx)
{
	...
	_mm_storel_epi64((__m128i *)(&drc), _mm_castpd_si128(_mm_mul_sd(exp_constant(_mm_mul_sd(vc, vc), ctx), ctx->v_discrete_normalisation)));
	...
	while (true)
	{
		...
		for (i = 0; i < 8; i++, ctx->head++)
		{
			if (ctx->head >= NORM_BATCH)
			{
				ctx->head = 0;
				box_muller(ctx);
			}

			vy = _mm_load_sd(ctx->norm + ctx->head);
			...
			if (dcmp)
			{
				...
				_mm_storel_epi64((__m128i *)(&drej), _mm_castpd_si128(exp_constant(_mm_mul_sd(_mm_add_sd(vyrc, vy), _mm_sub_sd(vyrc, vy)), ctx)));
...
void discrete_gaussian_vec(int64_t* samples, const double center, const size_t size, discrete_gaussian_ctx_t ctx) {
	for (size_t i = 0; i < size; i++) {
		samples[i] = discrete_gaussian(center, ctx);
	}
}
...
int main()
{
	...
	discrete_gaussian_ctx_t ctx;
	discrete_gaussian_ctx_init(ctx, 2.0);
	...
	for (t = 0; t < T; t++)
	{
		...
		for (i = 0; i < TOTAL; i++)
		{
			sample[i] = discrete_gaussian(center[i], ctx);
		}
		cycle2 = cpucycles();
	...
	discrete_gaussian_ctx_clear(ctx);
}
#endif
